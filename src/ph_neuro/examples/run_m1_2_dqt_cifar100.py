#!/usr/bin/env python3
"""Milestone M1.2 — DQT CNN on CIFAR-100 (GO/NO-GO >55%).

Second DQT milestone: scale the M1.1 DQT CNN up to the harder CIFAR-100
(100 classes, 50K train images). Uses a LARGER 3-conv
:func:`ph_neuro.models.dqt_models.dqt_cnn_cifar100` built from
:class:`TernaryDQTConv2d` (int8 ternary weights + stochastic rounding) for
the conv blocks and :class:`TernaryDQTLinear` for the classifier.

The STE baseline (E009/L1) for CIFAR-100 is 38.2% with a small 2-conv CNN;
DQT must beat it by ~17 points (target >55%).

The critical DQT mechanic (unchanged from M1.1, fully validated): after
EVERY ``optimizer.step()`` we call ``apply_dqt_rounding()`` on every DQT
layer to discretize the float accumulation buffer into int8 ternary
weights. For the first ``ANNEAL_FRACTION`` (0.80) of epochs this uses
stochastic rounding (exploration); the final 20% anneals to deterministic
``sign()`` so the network enters a clean fine-tuning regime (this is the
M1.1-RETRY fix that eliminated the late-training flip jitter).

Usage::

    python -m ph_neuro.examples.run_m1_2_dqt_cifar100 \\
        --lr 0.01 --epochs 150 --seed 42 --batch-size 128

    # in-process LR sweep over {0.01, 0.005, 0.001} with one seed:
    python -m ph_neuro.examples.run_m1_2_dqt_cifar100 \\
        --lr-sweep --epochs 150 --seed 42

Output:
    JSON file: ``{output_dir}/results_dqt_cifar100_lr{lr}_seed{seed}.json``
"""
...
from ph_neuro.training.data import get_cifar100_loaders
...
# 0.80 validated in M1.1-RETRY (E021.2) — gives a 30-epoch deterministic
# tail for 150 epochs, well within the patience-30 window.
ANNEAL_FRACTION = 0.80

# In-process LR sweep candidates (also runnable from scripts/ shell script).
SWEEP_LRS = [0.01, 0.005, 0.001]

# CIFAR-100 STE baseline (E009/L1, 2-conv CNN, 150 ep).
STE_BASELINE_ACCURACY = 0.382
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Milestone M1.2: DQT CNN on CIFAR-100 (GO/NO-GO >55%)"
    )
    parser.add_argument("--lr", type=float, default=0.01,
                        help="Learning rate (DQT best: 0.01)")
    parser.add_argument("--epochs", type=int, default=200)  # M1.2-RETRY: 150 -> 200
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--weight-decay", type=float, default=1e-4)
    parser.add_argument("--patience", type=int, default=40,  # M1.2-RETRY: 30 -> 40 (> anneal_start_epoch = 160)
                        help="Early stopping patience (must be > anneal_start_epoch)")
    ...
    parser.add_argument("--output-dir", default="results/phase1/m1_2_results")
    parser.add_argument(
...
def run_experiment(args: argparse.Namespace) -> None:
    """Run a single DQT CIFAR-100 experiment and save the result JSON."""
    ...
    print_header(
        f"M1.2 DQT CNN CIFAR-100 (GO/NO-GO >55%): "
        f"lr={args.lr}, {args.epochs}ep, seed={args.seed}, anneal@{int(ANNEAL_FRACTION * 100)}%"
    )
    ...
    print(
        f"CIFAR-100: {len(train_loader.dataset)} train, "
    ...
    model = dqt_cnn_cifar100(device=device, dtype=dtype)
    ...
    print(f"  Conv layers: TernaryDQTConv2d(3→64) → TernaryDQTConv2d(64→128) "
          f"→ TernaryDQTConv2d(128→256) (int8 ternary, no latent scores)")
    print()
    ...
    result = {
        ...
        "architecture": "DQT CNN Large: Conv(3→64→128→256)→Pool→FC(4096→512)→FC(512→100)",
        "method": "Direct Quantized Training (DQT) with stochastic rounding annealed to deterministic sign for final 20% (conv + linear)",
        "n_parameters": n_params,
        "peak_gpu_memory_mb": peak_mem_mb,
        "ste_baseline_accuracy": STE_BASELINE_ACCURACY,  # E009/L1 CIFAR-100 ternary STE
        **results,
    }
    ...
    output_path = os.path.join(
        args.output_dir,
        f"results_dqt_cifar100_lr{args.lr:g}_seed{args.seed}.json",
    )
    ...
    print()
    print(f"  STE Baseline (E009): {100 * STE_BASELINE_ACCURACY:.1f}%")
    if result["best_accuracy"] > 0.55:
        verdict = "GO ✅ (>55%)"
    elif result["best_accuracy"] > 0.50:
        verdict = "MARGINAL 🟡 (50-55%)"
    else:
        verdict = "NO-GO 🔴 (<=50%)"
    print(f"  M1.2 Verdict:        {100 * result['best_accuracy']:.2f}%  →  {verdict}")
    print()
...
def main() -> None:
    ...
    if args.lr_sweep:
        print_header(
            f"M1.2 DQT CNN CIFAR-100 — LR sweep over {SWEEP_LRS} "
            f"(seed={args.seed}, {args.epochs}ep)"
        )
        for lr in SWEEP_LRS:
            sweep_args = copy.copy(args)
            sweep_args.lr = lr
            sweep_args.lr_sweep = False
