...
> **Status:** Infrastructure (Phases 0–2.5) COMPLETE ✅ → **Brain Phase 0 COMPLETE ✅ → Brain Phase 1.1 COMPLETE 🟡 (PARTIAL SUCCESS) → Brain Phase 1.2 COMPLETE ❌ (LOW-RANK HEBBIAN REJECTED; LoRA bound met) → Brain Phase 1.3 COMPLETE ❌ (PREDICTIVE CODING INERT; LOCAL-RULE QUESTION CLOSED) → Brain Phase 2.1 COMPLETE ✅ (SURPRISE GATE ADDS VALUE) → Brain Phase 2.2 COMPLETE ✅ (TERNARY LORA: 2-BIT PRESERVES ≥90% OF FLOAT AT 16×) → Brain Phase 2.3 COMPLETE ❌ (CONSOLIDATION NEGATIVE — SPARSE TOP-K FAILS ON DENSE-SIGN ADAPTER) → next: Phase 3 (multi-domain at scale, scaling laws, vision)**
...
| **2.3** | Consolidation Mechanism | Sleep-inspired memory transfer: important plastic changes move to long-term store with slower decay. Does this reduce forgetting across sequential domains? | ❌ **CONSOLIDATION NEGATIVE (E036, 2026-08-16)** — sparse top-K=10% |ΔW| LT + warm-start ST: C's D3 +0.020±0.001 vs B1 fresh +0.178±0.034 (forward transfer −0.158, p=0.015; C/B1=0.112), ~2× slower plateau; BT≈0 but vacuous (LT retains ~1% of injection — dense-sign T-C × sparse transfer incompatible); storage wins (0.33×/0.93×) by discarding knowledge. Full-retention B2 has the only positive D3 (+0.187) but BT_D1 +0.265. Validated product = per-domain adapters (B1). [Report](docs/brain/10-e036-consolidation.md) |
...
> **Brain Step 2.3 (E036): ✅ COMPLETE — CONSOLIDATION NEGATIVE: SPARSE TOP-K FAILS ON THE DENSE-SIGN T-C ADAPTER (2026-08-16).**
> **Result (15 cells, 0 failures, pre-registered):** the last brain mechanism — sleep-like consolidation (long-term store + per-domain short-term) — on a 3-domain sequence (WikiText-2 → PubMed → CNN → **C4**, a new ODC-BY domain, frozen ppl 13.57). **C's D3 Δppl = +0.020 ± 0.001 vs B1's fresh +0.178 ± 0.034: forward transfer −0.158 ± 0.034 (p=0.015), C/B1 = 0.112** (the sparse-LT warm-start actively hurts D3; plateau ~2× slower). C's BT ≈ 0 passes the 0.1 bar but **vacuously** (the top-10% LT captures ~1% of the injection — D1/D2 effects +0.000/+0.007). C's storage wins (0.333× deployed / 0.931× LT+deltas) by discarding the learned knowledge. **Mechanistic cause:** T-C sign patterns are 100% dense and every nonzero entry contributes ±1 regardless of magnitude → |ΔW| magnitude is a poor importance proxy; a rank-1 outer product loses ~(1−K)² of its injection under sparsity. The only positive forward transfer comes from **full retention** (B2 continuing: D3 +0.187 > +0.178) but B2 pays real 3-domain interference (**BT_D1 = +0.265 > 0.1** — E035's near-zero 2-domain BT does not extend to 3). **Verdict: consolidation as designed does NOT add value on the product path; the Phase-2 validated stack is surprise gate + ternary storage + selectivity (B1-style per-domain adapters). Phase 2 complete (all four mechanisms tested) — ready for Phase 3 (multi-domain at scale, scaling laws, vision).** Full report [10-e036-consolidation.md](docs/brain/10-e036-consolidation.md).
