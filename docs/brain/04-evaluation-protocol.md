...
| 2026-08-14 | **Step 1.3 scope re-scoped (explicit, not silent) — predictive coding.** The Step 1.3 plan in `ROADMAP.md`/`BRAIN.md` was **Architectural Generalization** (repeat the vector-bias experiment on GPT-2 124M). E032's verdict (§12 of [06-e032-capacity-gain.md](06-e032-capacity-gain.md)) explicitly queued **error-based local rules** (predictive coding / target propagation) as the highest-value next test — the only remaining local-rule family with a credit-assignment story and the only path that could close the ~3-order-of-magnitude gap to the matched-budget LoRA bound (+1.52). Step 1.3 (E033) therefore runs the **predictive-coding experiment on the primary model** (SmolLM2-1.7B) at the same matched rank-1 344K budget as the E032 LoRA baseline, with the surprise modulator retained as the gate. This is the **final local-rule experiment** (pre-registered kill criteria in `07-e033-predictive-coding.md` §8: if PC fails, the local-rule question is CLOSED and the project pivots to the backprop-LoRA product path). The GPT-2 architectural generalization remains a possible cheap follow-up only if PC is positive. The **measurement protocol is unchanged** (metric, domains, budgets, window/stride, baselines, statistics, thresholds). | E032's verdict identified credit assignment as the missing ingredient and queued error-based local rules; the re-scoping answers that question definitively before any product-path pivot. Recorded as a deviation so the re-scoping is not silent. See [07-e033-predictive-coding.md](07-e033-predictive-coding.md) §9. | |
| 2026-08-14 | **Phase 2 scope re-scoped (explicit, not silent) — surprise-gated LoRA (E034, Step 2.1).** The Phase 2 plan in `ROADMAP.md`/`BRAIN.md` was **"local low-rank plastic matrices (more capacity → better adaptation)"**. E032 + E033 falsified that for **local rules** at the matched 344K budget (E032 low-rank Hebbian: destructive −1.35; E033 predictive coding: stable but inert +0.001), and E033's pre-registered consequence pivoted the project to the **backprop-LoRA product path** (E032's proven +1.52, source improved) as the Phase 2 adaptation/scaling mechanism. Step 2.1 (E034) therefore tests the **first value-add of the brain machinery on top of backprop LoRA**: the **surprise gate** (E031's one validated mechanism) applied as a **gated LoRA learning rate** (`effective lr = η·M_t`, M = sigmoid on relative loss deviation from EMA, M=0 during WikiText warmup). The **measurement protocol is unchanged** (metric, window/stride, budgets, baseline reuse, statistics, thresholds); the **new elements** are (a) a gated-lr update on the LoRA baseline and (b) a **second evaluation domain — CNN/DailyMail** (`abisee/cnn_dailymail` config `3.0.0`, license **apache-2.0**; frozen ppl 11.971, a moderate +12.3% vs WikiText-2 / +4.5% vs PubMed shift in the sigmoid's sensitive range) — for the **sequential two-domain selectivity test** (WikiText warmup → PubMed 100K → CNN/DailyMail 100K; pre-registered claim: gated LoRA's backward transfer on PubMed > plain LoRA's). Recorded so the re-scoping is not silent. See [08-e034-surprise-gated-lora.md](08-e034-surprise-gated-lora.md) §8. | E033's kill-criteria consequence (pivot to the backprop-LoRA product path) makes the Phase 2 scaling mechanism backprop LoRA, not local rules; E034 tests whether the surprise gate adds value on top of it (the first brain-machinery value-add). Recorded as a deviation so the re-scoping is not silent. | |
| 2026-08-15 | **Step 2.2 (E035, ternary LoRA) — two recorded notes (explicit, not silent).** (1) **Mechanism, not measurement:** the adapter representation changes to ternary {-1,0,+1} + per-matrix scale factors (T-A post-training quantize / T-B DQT / T-C STE); the **measurement protocol is unchanged** (metric, window/stride, budgets, baseline reuse, statistics, thresholds). Pre-registered in `09-e035-ternary-lora.md` §1/§7. (2) **T-B DQT-init amendment (pre-run, 2026-08-15):** T-B uses `ste_dqt.py`'s actual init convention (A and B ~ `N(0, 0.1)`, ≈10% nonzero int8 at construction) instead of the E034 `B = 0` identity, because an all-zero B cannot wake up a DQT adapter (`apply_stochastic_rounding()` flips one entry at a time — measured dead-start on a smoke run before the full cells). The **frozen-baseline I1 guarantee still holds exactly** because eval disables the adapter hooks. T-A and T-C keep exact `B = 0` identity at construction. Recorded so the amendment is not silent. See [09-e035-ternary-lora.md](09-e035-ternary-lora.md) §1/§8. | T-B's all-zero-B init was measured to be inert (DQT sparse-flip dynamics need ~10% nonzero weights to wake up — the same reason `ste_dqt.py` itself inits `N(0, 0.1)`); honoring it would have made T-B a trivial negative instead of a fair DQT test. Recorded as a protocol note (mechanism/init change, not a measurement change). | |
| 2026-08-15 | **Step 2.3 (E036, consolidation) — two recorded notes (explicit, not silent).** (1) **Mechanism + a third domain, not a measurement change:** the consolidation mechanism (long-term store + per-domain short-term, top-K |ΔW| sparse transfer) is an adapter-management change; the **measurement protocol is unchanged** (metric, window/stride, budgets, statistics, thresholds). A **third evaluation domain — C4** (`allenai/c4` config `en`, license **ODC-BY**, frozen ppl **13.568** on a deterministic 500K validation subsample) is added so the 3-domain sequence opens the surprise gate at every boundary (legal corpora SCOTUS 8.41 / LEDGAR 8.46 / EUR-LEX 6.51 were *easier* than the source → gate closed → vacuous). (2) **Consolidation design choices pre-registered before running:** K = 10% (top decile of |ΔW| over the global 344K budget), **add** transfer rule, **no LT decay**, warm-start = copy LT into ST (latents + scales; LT keeps canonical init scales), optimizer state cleared at each ST reset. These are mechanism-design choices, not measurement changes. Pre-registered in `10-e036-consolidation.md` §1/§4/§7. | E036 is the last brain mechanism on the product path; the D3 choice and the consolidation rule are documented so the negative forward-transfer result (C's D3 +0.020 vs B1's +0.178, p=0.015) is attributable to the mechanism (dense sign pattern × sparse transfer), not to a silent protocol change. Recorded so the amendment is not silent. | |
