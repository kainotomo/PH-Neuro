...
> **Phase 2.3 verdict (E036 — Consolidation):** **the pre-registered sleep-like consolidation (top-K=10% by |ΔW| sparse transfer into a long-term store + warm-started short-term) is NEGATIVE on the product path — the dense-sign T-C adapter is incompatible with sparse top-K transfer.** 3-domain sequence (WikiText-2 → PubMed → CNN → **C4**), 3 seeds, 15 cells, 0 failures. C's D3 Δppl = **+0.020 ± 0.001** vs B1's fresh **+0.178 ± 0.034** — **forward transfer −0.158 ± 0.034 (p=0.015), C/B1 = 0.112** (the sparse-LT warm-start actively hurts D3), plateau ~2× slower (360 vs 176 steps). C's BT ≈ 0 passes but **vacuously** (the top-10% store retains ~1% of the injection — LT's D1/D2 effects +0.000/+0.007); C's storage wins (0.333×/0.931×) by discarding the learned knowledge. **Mechanistic cause:** T-C sign patterns are 100% dense; every nonzero entry contributes ±1 regardless of magnitude, so |ΔW| magnitude is a poor importance proxy and a rank-1 outer product loses ~(1−K)² of its injection under sparsity. The only positive forward transfer comes from **full retention** (B2 continuing: D3 +0.187 > +0.178) but B2 pays real 3-domain interference (**BT_D1 = +0.265 > 0.1**, extending E035's 2-domain −0.0118). **Verdict: consolidation as designed does NOT add value; the Phase-2 validated stack is surprise gate + ternary storage + selectivity (B1-style per-domain adapters).** See [10-e036-consolidation.md](10-e036-consolidation.md).
...
| 2.3 | [Consolidation Mechanism](10-e036-consolidation.md) | Can we build long-term memory from short-term plasticity? | ❌ **CONSOLIDATION NEGATIVE (E036) — SPARSE TOP-K FAILS ON DENSE-SIGN ADAPTER.** Top-K=10% |ΔW| LT + warm-start ST: C's D3 +0.020 vs B1 fresh +0.178 (forward transfer −0.158, p=0.015; C/B1=0.112), plateau ~2× slower; BT≈0 but vacuous (LT retains ~1% of injection); storage wins (0.33×/0.93×) by discarding knowledge. Dense sign pattern × sparse transfer incompatible. B2 (full retention) shows the only positive D3 transfer (+0.187) but pays BT_D1 +0.265. Phase 2 validated stack = surprise gate + ternary + selectivity (B1 per-domain adapters). |
...
- ~~Consolidation reduces forgetting across sequential domains (Step 2.3, E036)~~ **FALSIFIED (E036)** — the sparse top-K |ΔW| consolidation gives **negative forward transfer** (−0.158, p=0.015) on the dense-sign T-C adapter (the LT retains ~1% of the injection); it only 'saves' storage by discarding knowledge. Full-retention continuing (B2) has the only positive D3 transfer but forgets D1 (BT +0.265) over 3 domains. The validated product is per-domain adapters (B1).
